...
if(isset($_SESSION['manufacturer_login'])) {
    $id = $_GET['id'];
    $pro_id = $orderQuantity = $availQuantity = array(); // Initialize as arrays
    ...
    // Check if variables are arrays and convert if necessary
    if (!is_array($orderProId)) {
        $orderProId = (array)$orderProId;
    }
    if (!is_array($availProId)) {
        $availProId = (array)$availProId;
    }
    if (!is_array($orderQuantity)) {
        $orderQuantity = (array)$orderQuantity;
    }
    if (!is_array($availQuantity)) {
        $availQuantity = (array)$availQuantity;
    }
